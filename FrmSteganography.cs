...
DSaveFilePath;
        ...
        Rectangle previewImage = new Rectangle(590, 100, 490, 470);
        ...
        {
            MessageBox.Show("Encrypting... Please wait");
...
(height * (width / 3) * 3) / 3 - fileNameSize - 1, true);
            ...
            if (FSize > 0)
            {
                ...
                {
                    changedBitmap = EncryptLayer(i, changedBitmap, (((8 - i) * height *
 (width / 3) * 3) / 3 - fileNameSize - (8 - i)), (((9 - i) * 	height * (width / 3) * 3) / 3 - fileNameSize - (9 - i)), false);
        ...
        private void FrmSteganography_Resize(object sender, EventArgs e)
        {
            Invalidate();
        }
        ...
        private void Encrypt_btnn_Click_1(object sender, EventArgs e)
        {
            if (saveFileDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                return;
            if (EnImage_tbxx.Text == String.Empty || EnFile_tbxx.Text == String.Empty)
            {
                MessageBox.Show("Encryption information is incomplete!\nPlease
                complete them first.", "Error", MessageBoxButtons.OK,   
                MessageBoxIcon.Error);
            }
            if (8 * ((height * (width / 3) * 3) / 3 - 1) < fileSize + fileNameSize)
            {
                MessageBox.Show("File size is too large!\nPlease use a larger image to hide 
                this file.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        ...
        private void EnImageBrowse_btnn_Click(object sender, EventArgs e)
        {
            if (openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                loadedTrueImagePath = openFileDialog1.FileName;
                EnImage_tbxx.Text = loadedTrueImagePath;
        ...
        private void EnFileBrowse_btnn_Click(object sender, EventArgs e)
        {
            if (openFileDialog2.ShowDialog() == DialogResult.OK)
            {
                loadedFilePath = openFileDialog2.FileName;
                EnFile_tbxx.Text = loadedFilePath;
        ...
        private void DeLoadImageBrowse_btnn_Click(object sender, EventArgs e)
        {
            if (openFileDialog3.ShowDialog() == DialogResult.OK)
            {
                DLoadImagePath = openFileDialog3.FileName;
                DeLoadImage_tbxx.Text = DLoadImagePath;
        ...
        private void DeSaveFileBrowse_btnn_Click(object sender, EventArgs e)
        {
            if (folderBrowserDialog1.ShowDialog() == DialogResult.OK)
            {
                DSaveFilePath = folderBrowserDialog1.SelectedPath;
                DeSaveFile_tbxx.Text = DSaveFilePath;
            }
        }

        private void Decrypt_btnn_Click_1(object sender, EventArgs e)
        {
            if (DeSaveFile_tbxx.Text == String.Empty || DeLoadImage_tbxx.Text == 
String.Empty)
            {
   ...
   MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            if (System.IO.File.Exists(DeLoadImage_tbxx.Text) == false)
            {
                MessageBox.Show("Select image file.", "Error", MessageBoxButtons.OK, 
   MessageBoxIcon.Exclamation);
                DeLoadImage_tbxx.Focus();
                return;
            }
            DecryptLayer();
        }
...
j++)
      ...
      startPosition + i * (height / 3) < fileSize + FNSize; i++)
        ...
        {
            MessageBox.Show("Decrypting... Please wait");
            ...
            for (; i < height && i * (height / 3) < fSize + fNameSize; i++)
                for (j = 0; j < (width / 3) * 3 && i * (height / 3) + (j / 3) < (height * (width / 
                            3) * 3) / 3 - 1 && i * (height / 3) + (j / 3) < fSize + fNameSize; j++)
                {
                    if (tempj != 0)
                    {
                        j = tempj;
            ...
            if (File.Exists(DSaveFilePath + "\\" + resFName))
            {
                ...
                another path to save file", "Error", MessageBoxButtons.OK, 
                MessageBoxIcon.Error);
                return;
            }
            else
